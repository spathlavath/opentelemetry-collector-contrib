...
func TestLogsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allEventsCount++
			lb.RecordNewrelicoracledbExecutionPlanEvent(ctx, timestamp, "query_id-val", "plan_hash_value-val", "query_text-val", "execution_plan_json-val")
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "newrelicoracledb.execution_plan":
					...
					attrVal, ok = lr.Attributes().Get("execution_plan_json")
					assert.True(t, ok)
					assert.Equal(t, "execution_plan_json-val", attrVal.Str())
				}
			}
		})
	}
