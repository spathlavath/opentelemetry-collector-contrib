...
for i in {1..5}; do
  ...
  curl -s -X POST "$BASE_URL/api/users" \
    ...
    -d "{\"name\":\"Test User $i\",\"email\":\"user$i@example.com\",\"role\":\"User\"}" | jq '.'
  echo ""
done
echo ""
...
curl -s "$BASE_URL/api/users/email/user1@example.com" | jq '.'
echo ""
echo ""
...
curl -s -X PUT "$BASE_URL/api/users/1" \
  -H "Content-Type: application/json" \
  -d '{"name":"Updated User","email":"updated@example.com","role":"Admin"}' | jq '.'
echo ""
echo ""
...
# Test long-running query endpoint (WARNING: This will take 5 minutes!)
...
curl -s "$BASE_URL/api/users/slow-query" > /dev/null &
SLOW_QUERY_PID=$!
echo "Slow query started with PID: $SLOW_QUERY_PID"
echo "This query should now be visible in the OTEL collector's query performance monitoring."
echo "To kill the query manually: kill $SLOW_QUERY_PID"
echo ""
echo ""
