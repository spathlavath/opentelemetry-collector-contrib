...
// ScrapeExecutionPlans fetches execution plans for specific SQL_ID + CHILD_NUMBER combinations
func (s *ExecutionPlanScraper) ScrapeExecutionPlans(ctx context.Context, sqlIdentifiers []models.SQLIdentifier) []error {
	...
	for _, identifier := range sqlIdentifiers {
		...
		// Fetch execution plan for specific SQL_ID and CHILD_NUMBER combination
		planRows, err := s.client.QueryExecutionPlanRows(queryCtx, identifier.SQLID, identifier.ChildNumber)
		if err != nil {
			errs = append(errs, fmt.Errorf("failed to query execution plans for SQL_ID %s CHILD_NUMBER %d: %w",
				identifier.SQLID, identifier.ChildNumber, err))
			continue // Skip this combination but continue processing others
		}
