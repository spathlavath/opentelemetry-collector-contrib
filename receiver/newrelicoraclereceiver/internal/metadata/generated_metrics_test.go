...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordNewrelicoracledbBlockingQueriesWaitTimeDataPoint(ts, 1, "newrelic.entity_name-val", "instance.id-val", "blocked.user-val", "blocking.user-val", "blocked.sql.id-val", "blocked.sid-val", "blocking.sid-val", "blocked.serial-val", "blocking.serial-val", "blocked.query.text-val", "database.name-val")
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "newrelicoracledb.blocking_queries.wait_time":
					...
					attrVal, ok = dp.Attributes().Get("blocked.sid")
					assert.True(t, ok)
					assert.Equal(t, "blocked.sid-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("blocking.sid")
					assert.True(t, ok)
					assert.Equal(t, "blocking.sid-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("blocked.serial")
					assert.True(t, ok)
					assert.Equal(t, "blocked.serial-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("blocking.serial")
					assert.True(t, ok)
					assert.Equal(t, "blocking.serial-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("blocked.query.text")
					assert.True(t, ok)
					assert.Equal(t, "blocked.query.text-val", attrVal.Str())
