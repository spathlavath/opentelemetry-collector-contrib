...
func (s *sqlServerScraper) scrape(ctx context.Context) (pmetric.Metrics, error) {
	...
	// Scrape execution plan operators for active queries (if query monitoring is enabled)
	if s.config.EnableQueryMonitoring && s.queryPerformanceScraper != nil {
		s.logger.Info("=== Starting execution plan collection for ACTIVE queries ===")
		...
		if err != nil {
			s.logger.Warn("Failed to fetch slow queries for correlation, skipping execution plan collection",
				zap.Error(err))
		} else if len(slowQueries) > 0 {
			// Extract query IDs and lightweight plan data from slow queries
			slowQueryIDs, slowQueryPlanDataMap := s.queryPerformanceScraper.ExtractQueryDataFromSlowQueries(slowQueries)
			s.logger.Info("Extracted query IDs and lightweight plan data",
				zap.Int("slow_query_count", len(slowQueries)),
				zap.Int("unique_query_id_count", len(slowQueryIDs)),
				zap.Int("plan_data_map_size", len(slowQueryPlanDataMap)))

			// Fetch active running queries matching slow query IDs
			limit := s.config.QueryMonitoringCountThreshold
			activeElapsedTimeThreshold := s.config.ActiveRunningQueriesElapsedTimeThreshold
			...
			if err != nil {
				...
				s.logger.Info("Active queries fetched, emitting execution plan metrics",
					zap.Int("active_query_count", len(activeQueries)))

				// Fetch execution plans and emit as metrics
				if err := s.queryPerformanceScraper.EmitActiveRunningExecutionPlansAsMetrics(ctx, activeQueries, slowQueryPlanDataMap); err != nil {
					s.logger.Error("Failed to emit execution plans as metrics", zap.Error(err))
					scrapeErrors = append(scrapeErrors, err)
				} else {
					s.logger.Info("Successfully emitted execution plan metrics")
				}
			} else {
				s.logger.Info("No active queries found matching slow query IDs")
			}
		...
		s.logger.Info("=== Completed execution plan collection for ACTIVE queries ===")
	}
