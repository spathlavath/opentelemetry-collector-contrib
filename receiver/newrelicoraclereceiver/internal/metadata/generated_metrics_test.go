...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordNewrelicoracledbSlowQueriesAvgRowsReturnedDataPoint(ts, 1, "collection_timestamp-val", "database_name-val", "query_id-val", "user_name-val", "normalised_sql_hash-val", "client_name-val", "transaction_name-val")
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "newrelicoracledb.slow_queries.avg_rows_examined":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "Average rows examined per execution for slow queries (buffer gets/logical reads)", ms.At(i).Description())
					...
					assert.True(t, ok)
					assert.Equal(t, "transaction_name-val", attrVal.Str())
				case "newrelicoracledb.slow_queries.avg_rows_returned":
					assert.False(t, validatedMetrics["newrelicoracledb.slow_queries.avg_rows_returned"], "Found a duplicate in the metrics slice: newrelicoracledb.slow_queries.avg_rows_returned")
					validatedMetrics["newrelicoracledb.slow_queries.avg_rows_returned"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "Average rows returned per execution for slow queries (rows processed)", ms.At(i).Description())
					assert.Equal(t, "{rows}", ms.At(i).Unit())
					dp := ms.At(i).Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
