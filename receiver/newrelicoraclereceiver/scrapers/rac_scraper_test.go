...
func TestNewRacScraper(t *testing.T) {
	...
	scraper := NewRacScraper(nil, mb, logger, instanceName, config)
...
func TestRacScraper_CachedRacMode(t *testing.T) {
	...
	t.Run("uninitialized", func(t *testing.T) {
		scraper := NewRacScraper(nil, mb, logger, "test-instance", metadata.DefaultMetricsBuilderConfig())
		assert.Nil(t, scraper.isRacMode)
	})
...
func TestRacScraper_MetricConfiguration(t *testing.T) {
	...
	t.Run("default_config", func(t *testing.T) {
		scraper := NewRacScraper(nil, mb, logger, "test-instance", config)
	...
	t.Run("custom_config", func(t *testing.T) {
		...
		scraper := NewRacScraper(nil, mb, logger, "test-instance", customConfig)
		assert.False(t, scraper.metricsBuilderConfig.Metrics.NewrelicoracledbAsmDiskgroupTotalMb.Enabled)
	})
}

func TestRacScraper_NilDatabase(t *testing.T) {
	settings := receivertest.NewNopSettings(metadata.Type)
	mb := metadata.NewMetricsBuilder(metadata.DefaultMetricsBuilderConfig(), settings)
	logger := zap.NewNop()
	config := metadata.DefaultMetricsBuilderConfig()
...
func TestRacScraper_MultipleScraper(t *testing.T) {
	...
	scraper1 := NewRacScraper(nil, mb1, logger, "instance-1", config)
	scraper2 := NewRacScraper(nil, mb2, logger, "instance-2", config)
...
func TestRacScraper_EmptyInstanceName(t *testing.T) {
	...
	scraper := NewRacScraper(nil, mb, logger, "", config)
