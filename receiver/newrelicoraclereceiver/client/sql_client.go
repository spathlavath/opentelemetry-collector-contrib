...
func (c *SQLClient) QueryLockCounts(ctx context.Context) ([]models.LockCount, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&lc.InstID, &lc.LockType, &lc.LockMode, &lc.LockCount); err != nil {
			return nil, err
		}
...
func (c *SQLClient) QueryLockSessionCounts(ctx context.Context) ([]models.LockSessionCount, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&lsc.InstID, &lsc.LockType, &lsc.SessionCount); err != nil {
			return nil, err
		}
...
func (c *SQLClient) QueryLockedObjectCounts(ctx context.Context) ([]models.LockedObjectCount, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&loc.InstID, &loc.LockType, &loc.ObjectType, &loc.ObjectCount); err != nil {
			return nil, err
		}
...
// QueryDeadlockCount executes the deadlock count query
func (c *SQLClient) QueryDeadlockCount(ctx context.Context) (*models.DeadlockCount, error) {
	row := c.db.QueryRowContext(ctx, queries.DeadlockCountSQL)

	var dc models.DeadlockCount
	if err := row.Scan(&dc.DeadlockCount); err != nil {
		return nil, err
	}
